...
function analyzeCarMileage(cars: Car[]): CarMileageAnalysis {
  const outputObject: CarMileageAnalysis = {
    averageMileage: 0,
  ...
  const averageMileage =
    cars.reduce((total, car) => total + car.mileage, 0) / cars.length;
